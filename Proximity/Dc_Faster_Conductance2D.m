function conductance_2D=Dc_Faster_Conductance2D(energy,life,energygap_factor,z_factor)

NPHI=10;
step_phi=pi/4/NPHI;
phi_quarter=0:step_phi:pi/4;

theta_up=1.50;
NTHETA=10;
half_step=theta_up/NTHETA;
theta=0:half_step:theta_up;
...
%     center(i)=quadgk(@(phi) Faster_Conductance1D(energy_center(i),far_super_gap_factor,zero_super_gap_factor,zero_normal_gap_factor,z_factor,phi),0,2*pi,'AbsTol',1e-6);
    conductance_seq=Dc_Faster_Conductance1D(energy_center(i),far_super_gap_factor,zero_super_gap_factor,zero_normal_gap_factor,z_factor,theta,phi_quarter);
    center(i)=trapz(phi_quarter,conductance_seq)*16;
...
conductance_2D=[left,center,right];
 integral_conductance_N=2*pi*quadgk(@(beta) sin(2*beta).*Conductance_N(beta,z_factor),0,theta_up);
