...
%     center(i)=quadgk(@(phi) Faster_Conductance1D(energy_center(i),far_super_gap_factor,zero_super_gap_factor,zero_normal_gap_factor,z_factor,phi),0,2*pi,'AbsTol',1e-6);
    conductance_seq=Dab_Faster_Conductance1D(energy_center(i),far_super_gap_factor,zero_super_gap_factor,zero_normal_gap_factor,z_factor,theta);
...
conductance_2D=[left,center,right];
 integral_conductance_N=quadgk(@(beta) cos(beta).*Conductance_N(beta,z_factor),-1.50,1.50);
