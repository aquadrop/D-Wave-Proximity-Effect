function conductance_seq=Dc_Faster_Conductance1D(energy,far_super_gap_factor,zero_super_gap_factor,zero_normal_gap_factor,z_factor,theta,phi)
control_S=6;
control_N=control_S;
beta_S_factor=38;
beta_N_factor=16.4;
x_S=200;%control_S*beta_S;
x_N=x_S/2;%control_N*beta_N;
% control_T=1.5;
...
beta_N=beta_N_factor.*ones(1,length(theta));%*cos(theta);%cos(theta)./cos(2*theta);%./EnergyGap(phi,1);ones(1,length(theta))
...
conductance_seq_theta=zeros(1,length(theta));
for i=1:length(phi)
...
%    fprintf('##phi at %d of %d## %f***\n',i,length(phi),phi(i));
   ...
   gap=abs(gap);
   k_S=(sqrt(energy.^2-far_super_gap(i).^2))./(beta_S(j));
   ...
   % Initial_Super_b=[0 0];
   [X_a,wave_a]=SuperSolve(energy,Initial_Super_a,gap,x_S,beta_S(j).*cos(theta(j)));
   ...
   [X_a,wave_a]=NormalSolve(energy,Initial_Super_a,gap,x_N,beta_N(j).*cos(theta(j)));
   [X_b,wave_b]=NormalSolve(energy,Initial_Super_b,gap,x_N,beta_N(j).*cos(theta(j)));
   % X_a2=X_a;
   % X_b2=X_b;
   % wave_a2=wave_a;
   % wave_b2=wave_b;
   ...
   % z=Z(theta,z_factor);
   conductance_seq_theta(j)=sin(2*theta(j)).*Transmission(Z(theta(j),z_factor),u_a,v_a,u_b,v_b);
   end
%    conductance_seq(i)=conductance_seq_theta(1);%
   conductance_seq(i)=trapz(theta,conductance_seq_theta);
