function conductance_2D=Dab_Faster_Conductance2D(energy,life,energygap_factor,z_factor)
NPHI=10000;
step_phi=2*pi/NPHI;
phi=0:step_phi:2*pi;
...
theta_up=1.5;
NTHETA=15;
half_step=pi/2/NTHETA;
theta_positive=0:half_step:theta_up;
theta_negative=-theta_positive(end:-1:2);
theta=[theta_negative,theta_positive];
...
%     center(i)=quadgk(@(phi) Faster_Conductance1D(energy_center(i),far_super_gap_factor,zero_super_gap_factor,zero_normal_gap_factor,z_factor,phi),0,2*pi,'AbsTol',1e-6);
    conductance_seq=Dab_Faster_Conductance1D(energy_center(i),far_super_gap_factor,zero_super_gap_factor,zero_normal_gap_factor,z_factor,theta_positive);
    center(i)=trapz(theta_positive,conductance_seq)*2;
...
conductance_2D=[left,center,right];
 integral_conductance_N=quadgk(@(beta) cos(beta).*Conductance_N(beta,z_factor),-theta_up,theta_up);
